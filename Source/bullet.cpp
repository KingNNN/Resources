...
Bullet::Bullet(SDL_Renderer *renderer, string filePath, float x, float y)
{
	...
	//initialize the direction vars from te bullet no X movement, only Y movement (up)
	xDir = 0;
	yDir = -1;
...
void Bullet::Update(float deltaTime)
{
	...
	if(active)
	{
		...
		//check to see if the bullet is off the top of the screen and deactivate and move off screen
